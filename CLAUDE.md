...
- **旧パス禁止（自動強制）:** `./data/` `./js/` `./index/` `./lexicon/` への参照は pre-commit hook（`scripts/path-audit.js`）と CI（`.github/workflows/path-check.yml`）がブロックする。移行先はそれぞれ `./assets/data/` / `./core/`・`./assets/js/` / `./assets/data/index/` / `./assets/data/lexicon/`。
