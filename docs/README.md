...
> ロードマップデータの単一ソース: [`public/assets/data/roadmap.json`](../public/assets/data/roadmap.json)
> アプリ内の ℹ️ → ロードマップ からも確認できます。

<!-- roadmap.json の内容を以下に反映（編集は roadmap.json のみ行うこと） -->

* テキストと辞書（読みを支える言語データを充実させます）
  * 日本語訳の追加（新改訳2017・聖書協会共同訳など現代語訳への対応）
  * ヘブライ語聖書への対応（旧約を読解フローで読む）
  * 辞書の拡充（読書中に引っかかりを減らす形で）
* 読書体験（読む流れを止めない一体感のある画面へ）
  * 画面を分けずに読む（本文・検索・調べるの統合）
  * 読んでいた場所への即時復帰（ジャンプ・戻る・履歴保持）
* 記録と持ち出し（読書の痕跡を残し持ち出せるようにします）
  * メモのつながり（タグではなく”参照関係”としての最小接続）
  * 読書記録のエクスポート（外部ツールへの持ち出し）
