...
## 12. 原語表記の原則（2026-07-11 制定）

読書メモの全表示面（Passage Note / Phrase Reading / Observation）に適用する。

> **読書メモは、日本語を入口とし、必要な場合のみ括弧内にギリシャ語を添える。**
> **ギリシャ語は本文中の対象語を特定するための「指さし」として用い、**
> **説明の主体にしてはならない。**
> **読者はまず日本語を読み、次に本文中のギリシャ語を見つけ、最後に本文へ戻る。**

### 「必要な場合」の判定

括弧内ギリシャ語（＝指さし）が**必要**なのは、
**文が言及する語がまだ画面上で特定されていない**場合だけである。

| 状況 | 指さし | 理由 |
|---|---|---|
| クリックした語自身への言及（Observation の引用「◯◯」） | 不要 | 対象語は選択済みで、読者は既にどの語か知っている |
| クリックした語と**対になる別の語**への言及（οὐ↔μή、μέν↔δέ） | **必要** | 読者はその語を本文から探す。日本語グロスでは特定できない（οὐ と μή はともに「〜ない」） |
| 節を開いた瞬間の節文（Passage Note）での語への言及 | **必要** | どの語も選択されていない |

### 書式

- 常に **日本語（ギリシャ語）** の順。例: 「もう一つの打ち消し（μή）」
- ギリシャ語が主語・目的語になる文（例:「μή と組み合わさり」）は禁止
- 説明の語彙は日本語で統一（「打ち消し」— 「否定辞」「二重否定」等の
  文法用語は使わない）

### 適用実績（2026-07-11 監査に基づく初回適用）

対象は監査で特定された8文のみ: `_WORD_ROLE_TEXT` の強調否定3文・
μέν…δέ 対比3文、`_VERSE_NOTE_TEXT` の強調否定・対比の2文。
それ以外の全文章（談話文・Wallace 語文・Phrase Reading 全27文型・
Observation 指さし文）は裸のギリシャ語を含まず、変更不要と確認済み。
